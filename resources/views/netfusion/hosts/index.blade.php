...
@section('title', __('netfusion.hotspot_hosts'))
...
@section('content')
        ...
        <div class="row g-3 mb-4 align-items-center justify-content-between">
            <div class="col-12 col-lg-auto">
                <h4 class="fw-bold text-dark mb-1 d-flex align-items-center gap-2">
                    {{ __('netfusion.hotspot_hosts') }}
                    <span
                        class="badge bg-primary bg-opacity-10 text-primary border border-primary fw-normal px-2 py-1 rounded-pill ms-2"
                        style="font-size: 0.75rem;">
                        {{ count($hosts) }}
                    </span>
                </h4>
                <div class="text-muted small d-flex align-items-center gap-2">
                    <i class="bi bi-display"></i> {{ __('netfusion.connected_devices') }}
                </div>
            </div>
            ...
            <div class="col-12 col-lg-auto d-flex gap-2 align-items-center">
                ...
                <div class="position-relative w-100" style="max-width: 300px;">
                    ...
                    <input type="text" id="searchInput"
                        ...
                        placeholder="{{ __('netfusion.search_host') }}" style="width: 100%;">
                </div>
            </div>
        </div>
        ...
        <div class="glass-card border-0 shadow-lg rounded-4 overflow-hidden position-relative" style="min-height: 400px;">
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light bg-opacity-50 text-secondary text-uppercase small ls-1 fw-bold border-bottom">
                        <tr>
                            ...
                            <th class="py-3">{{ __('netfusion.address_ip') }}</th>
                    ...
                    <tbody class="border-top-0">
                        @forelse($hosts as $host)
                            <tr class="user-item"
                                data-search="{{ strtolower(($host['mac-address'] ?? '') . ' ' . ($host['address'] ?? '') . ' ' . ($host['comment'] ?? '')) }}">
                                <td class="ps-4">
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <span class="fw-bold text-dark font-monospace">{{ $host['mac-address'] ?? '-' }}</span>
                                    </div>
                                </td>
                                <td class="font-monospace text-muted">{{ $host['address'] ?? '-' }}</td>
                                <td class="font-monospace text-muted">{{ $host['to-address'] ?? '-' }}</td>
                                <td>
                                    <span
                                        class="badge bg-light text-dark border">{{ $host['server'] ?? __('netfusion.server_all') }}</span>
                                </td>
                                <td class="text-muted small">
                                    @if(isset($host['comment']))
                                        <i class="bi bi-chat-text me-1 text-secondary"></i> {{ $host['comment'] }}
                                    @else
                                        <span class="text-muted opacity-50">-</span>
                                    @endif
                                </td>
                                <td class="text-end pe-4">
                                    <form action="{{ route('mikrotik-suite.netfusion.hosts.destroy', $host['.id']) }}" method="POST"
                                        class="d-inline"
                                        onsubmit="return confirm('{{ __('netfusion.confirm_remove_host') }}');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="btn btn-sm btn-light text-danger border hover-shadow rounded-circle p-2"
                                            title="{{ __('netfusion.remove_host') }}">
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-5">
                                    <div class="d-flex flex-column align-items-center opacity-50">
                                        <i class="bi bi-display fs-1 mb-2"></i>
                                        <h6>{{ __('netfusion.no_connected_hosts') }}</h6>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="d-block d-lg-none p-3 position-relative" style="z-index: 1;">
                        ...
                        <div class="card-body p-3">
                            <div class="d-flex justify-content-between align-items-start mb-3">
                                <div class="d-flex align-items-center gap-3">
                                    ...
                                    <div>
                                        <h6 class="fw-bold text-dark mb-0 font-monospace">{{ $host['mac-address'] ?? '-' }}</h6>
                                        <span
                                            class="badge bg-light text-dark border">{{ $host['server'] ?? __('netfusion.server_all') }}</span>
                                    </div>
                                </div>
                                <form action="{{ route('mikrotik-suite.netfusion.hosts.destroy', $host['.id']) }}" method="POST"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-light text-danger border rounded-circle p-2 shadow-sm"
                                        style="width: 36px; height: 36px;">
                                        <i class="bi bi-trash"></i>
                                    </button>
                                </form>
                                ...
                                <div class="col-6">
                                    <div class="bg-light rounded p-2 border border-light">
                                        <span class="d-block text-muted"
                                            style="font-size: 10px;">{{ strtoupper(__('netfusion.address_ip')) }}</span>
                                        <span class="font-monospace fw-bold">{{ $host['address'] ?? '-' }}</span>
                                    </div>
                                </div>
                                <div class="col-6">
                                    <div class="bg-light rounded p-2 border border-light">
                                        <span class="d-block text-muted"
                                            style="font-size: 10px;">{{ strtoupper(__('netfusion.to_address')) }}</span>
                                        <span class="font-monospace fw-bold">{{ $host['to-address'] ?? '-' }}</span>
                                    </div>
                                </div>
                                @if(isset($host['comment']))
                                    <div class="col-12 mt-1">
                                        <div class="d-flex align-items-center text-muted small">
                                            <i class="bi bi-chat-text me-2"></i> {{ $host['comment'] }}
                                        </div>
                                    </div>
                                @endif
                            </div>
                ...
                <div id="noSearchResults" class="text-center py-5 d-none">
                    ...
                    <h6 class="text-muted">{{ __('netfusion.no_hosts_found_query') }}</h6>
                </div>
            </div>
            ...
            <div class="card-footer bg-transparent border-top p-3 d-flex justify-content-between align-items-center small text-muted position-relative"
                style="z-index: 1;">
                <span id="showingCount">{{ __('netfusion.showing_hosts', ['count' => count($hosts)]) }}</span>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('searchInput').addEventListener('keyup', function () {
            ...
            if (showingEl) showingEl.innerText = "{{ __('netfusion.showing_hosts', ['count' => '']) }}" + visibleCount;
        });
    </script>
