...
@section('content')
    <x-page-header title="{{ __('mikrotik-suite.network.load_balancing.nth.title') }}"
        subtitle="{{ __('mikrotik-suite.network.load_balancing.nth.subtitle') }}">
        <x-slot name="action">
            <div class="d-flex align-items-center justify-content-center gap-2 px-3 py-2 rounded-pill bg-primary bg-opacity-10 border border-primary border-opacity-10 text-primary small fw-medium text-nowrap mx-auto ms-md-auto"
                ...
                <i class="bi bi-123 fs-6"></i>
                <span>{{ __('mikrotik-suite.network.load_balancing.nth.title') }}</span>
            </div>
        </x-slot>
    </x-page-header>
    ...
    <div class="row g-4 mb-3 mb-md-5">
        ...
        <div class="col-lg-5">
            <div class="glass-card-static p-4">
                ...
                <form id="nthForm" class="no-loader"
                    data-route="{{ route('mikrotik-suite.network.load-balancing.nth.generate') }}">
                    {{-- Global Settings --}}
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <select class="form-select glass-select" name="local_type" id="localType">
                                    <option value="address-list">
                                        {{ __('mikrotik-suite.network.load_balancing.form.options.ip_address_list') }}</option>
                                    <option value="interface">
                                        {{ __('mikrotik-suite.network.load_balancing.form.options.in_interface') }}</option>
                                    <option value="interface-list">
                                        {{ __('mikrotik-suite.network.load_balancing.form.options.in_interface_list') }}</option>
                                </select>
                            </div>
                            <div class="col-12 col-md-6">
                        ...
                        <table class="table table-glass align-middle mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="py-2 text-uppercase text-center" style="width: 25%;">
                                        {{ __('mikrotik-suite.network.load_balancing.form.weight') }}</th>
                                    <th class="py-2 text-uppercase text-center" style="width: 25%;">
...
@push('scripts')
    @vite(['resources/js/pages/mikrotik-suite/network/load-balancing/nth.js'])
@endpush
