...
@section('content')
    <x-page-header title="{{ __('mikrotik-suite.network.load_balancing.pcc.title') }}"
        subtitle="{{ __('mikrotik-suite.network.load_balancing.pcc.subtitle') }}">
        <x-slot name="action">
            <div class="d-flex align-items-center justify-content-center gap-2 px-3 py-2 rounded-pill bg-primary bg-opacity-10 border border-primary border-opacity-10 text-primary small fw-medium text-nowrap mx-auto ms-md-auto"
                ...
                <span>{{ __('mikrotik-suite.network.load_balancing.pcc.title') }}</span>
            </div>
        </x-slot>
    </x-page-header>
    ...
    <div class="row g-4 mb-3 mb-md-5">
        ...
        <div class="col-lg-5">
            <div class="glass-card-static p-4">
                ...
                <form id="pccForm" class="no-loader"
                    data-route="{{ route('mikrotik-suite.network.load-balancing.pcc.generate') }}">
                    {{-- Global Settings --}}
                    <div class="row g-3 mb-4">
                        <div class="col-md-6">
                            ...
                            <input type="number" class="form-control glass-input" name="wan_count" id="wanCount" value="2"
                                min="2" max="20" placeholder="{{ __('mikrotik-suite.network.load_balancing.form.wan_lines') }}">
                        </div>
                        <div class="col-md-6">
                            <label
                    ...
                    <div class="mb-4">
                        <div class="row g-2">
                            <div class="col-12 col-md-6">
                                <label
                                    class="form-label text-secondary small fw-bold text-uppercase">{{ __('mikrotik-suite.network.load_balancing.form.local_target_type') }}</label>
                                <select class="form-select glass-select" name="local_type" id="localType">
                                    <option value="address-list">
                                        {{ __('mikrotik-suite.network.load_balancing.form.options.ip_address_list') }}
                                    </option>
                                    <option value="interface">
                                        {{ __('mikrotik-suite.network.load_balancing.form.options.in_interface') }}
                                    </option>
                                    <option value="interface-list">
                                        {{ __('mikrotik-suite.network.load_balancing.form.options.in_interface_list') }}
                                    </option>
                                </select>
                            </div>
                            <div class="col-12 col-md-6">
                                <label class="form-label text-secondary small fw-bold text-uppercase"
                                    id="localTargetLabel">{{ __('mikrotik-suite.network.load_balancing.form.local_ip_target') }}</label>
                                <input type="text" class="form-control glass-input" name="local_target" id="localTarget"
                                    value="192.168.0.0/16, 172.16.0.0/12, 10.0.0.0/8" placeholder="e.g. 192.168.88.0/24">
                            </div>
                        </div>
                    </div>
                            ...
                            <thead>
                                <tr>
                                    <th class="py-2 text-uppercase text-center" style="width: 25%;">
                                        {{ __('mikrotik-suite.network.load_balancing.form.wan_interface') }}
                                    </th>
                                    <th class="py-2 text-uppercase text-center" style="width: 25%;">
                                        {{ __('mikrotik-suite.network.load_balancing.form.gateway_ip') }}
                                    </th>
                                    <th class="py-2 text-uppercase text-center" style="width: 25%;">
                                        {{ __('mikrotik-suite.network.load_balancing.form.speed_mbps') }}
                                    </th>
                                    <th class="py-2 text-uppercase text-center" style="width: 25%;">
                                        {{ __('mikrotik-suite.network.load_balancing.form.ip_dns_check') }}
                                    </th>
                                </tr>
                            </thead>
            ...
            <div class="glass-card-static h-100 d-flex flex-column overflow-hidden">
                <div class="d-flex justify-content-between align-items-center px-3 py-2 border-bottom border-dark bg-dark">
                    ...
                    <button class="btn btn-sm btn-dark border border-secondary text-light fw-bold py-0"
                        ...
                        <i class="bi bi-clipboard me-1"></i> {{ __('netfusion.copy') }}
                    </button>
                </div>
...
@push('scripts')
    @vite(['resources/js/pages/mikrotik-suite/network/load-balancing/pcc.js'])
@endpush
