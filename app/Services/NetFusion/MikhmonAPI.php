...
/*****************************
 *
 * RouterOS PHP API class v1.6 (Ported for NetFusion)
 * Author: Denis Basta
 * Contributors: ... (Mikhmon Version)
 *
 ******************************/
...
class MikhmonAPI
{
    ...
    public $timeout = 10;    //  Connection attempt timeout and data read timeout (Default 3, specific increase to 10 for NetFusion)
    ...
    {
        if ($this->debug) {
            // Log to Laravel log instead of echo
            \Illuminate\Support\Facades\Log::info("MikhmonAPI: " . $text);
        }
    }
    ...
    public function encodeLength(int $length): string
    {
        if ($length < 0x80) {
            ...
            $length |= 0x8000;
            $result = chr(($length >> 8) & 0xFF) . chr($length & 0xFF);
        } elseif ($length < 0x200000) {
            $length |= 0xC00000;
            $result = chr(($length >> 16) & 0xFF) . chr(($length >> 8) & 0xFF) . chr($length & 0xFF);
        } elseif ($length < 0x10000000) {
            $length |= 0xE0000000;
            $result = chr(($length >> 24) & 0xFF) . chr(($length >> 16) & 0xFF) . chr(($length >> 8) & 0xFF) . chr($length & 0xFF);
        } else {
            $result = chr(0xF0) . chr(($length >> 24) & 0xFF) . chr(($length >> 16) & 0xFF) . chr(($length >> 8) & 0xFF) . chr($length & 0xFF);
        }
    ...
    {
        if (!is_resource($this->socket)) {
            $this->debug('Read Error: Socket is not connected/valid resource.');
            return array();
        }
    ...
    {
        if (!is_resource($this->socket)) {
            $this->debug('Write Error: Socket is not connected/valid resource.');
            return false;
        }
